...
int main(){
    ...
    printQueue(Q);  
    // Expected: Rear -> [50][40][30][20] <- Front 
    // (front = 20, rear = 50)
...
void dequeue(LLQueue* Q){
    if (isEmpty(*Q) == true){
        ...
        LIST* trav; 
        for (trav = &Q->rear; (*trav)->next != NULL; trav = &(*trav)->next){}
        LIST temp = *trav;
        *trav = NULL;
        free(temp);
...
void printQueue(LLQueue Q) {
    printf("REAR -> ");
    for (LIST trav = Q.rear; trav != NULL; trav = trav->next) {
        printf("[%d] ", trav->data);
    }
    printf("<- FRONT\n");
}
