...
void populateAlbum(Album *ae){
    ...
    for(int i=0; i < 15; i++){
        ...
        printf("\nAdded song to album: %s (%d)", library.List[i].songTitle, library.List[i].releaseYear);
    }
    printf("\n");
}
...
void pushSong(Album *ae, Song newSong){
	// Implement your code here...
    SongStack newNode = malloc(sizeof(SongNode));
    if(newNode!=NULL){
...
void popSong(Album *ae){
    ...
    if(isEmpty(ae)){
        ...
        Song popped;
        popped = temp->songInfo;
        ae->top = ae->top->nextSong;
        printf("Removed song: %s (%d)\n", popped.songTitle, popped.releaseYear);
        free(temp);
    } 
