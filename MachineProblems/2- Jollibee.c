...
int main (void)
{
	...
	*/ // DONE
	...
	Customer cust1 = custList[0], cust2 = custList[1];
	...
	*/
	...
	displayMealOrder(custOrder1);
	deliverMeals(&custOrder1, deliverOrds1, 1);
	displayMealOrder(custOrder1);
...
void addMealToOrder(MealOrder* MO, Menu menuList, Meal newOrder)
{
	...
	// check if meal exists in menu
	for(i = 0; i < MENUSIZE && strcmp(newOrder.mealCode, menuList.menuItems[i].mealCode) != 0; i++){}

	if(i < MENUSIZE){ // meal exists in menu
		// check if meal exists in order
		for(trav = &MO->ML; *trav != NULL && strcmp((*trav)->order.mealCode, newOrder.mealCode) != 0; trav = &(*trav)->link){}
		
		if(*trav != NULL){ // meal already in order
			(*trav)->qty++;
		} else { // meal not yet in order
			newNode = (MealList)malloc(sizeof(MealNode));
	...
	} else { // meal does not exists in menu
		printf("\nWe don't have the %s in our store.\n", newOrder.mealName);
	}
...
// for each meals in order:
// 	if meal delQty matches qty:
// 		update mealStat to 1
 		
void checkMealCompletion(MealOrder MO)
{
	MealList trav;
	
	for(trav = MO.ML; trav != NULL; trav = trav->link){ // traverse each meal in order
		if(trav->delQty == trav->qty){ // all meals are delivered
			trav->mealStat = 1;
		}
	}
}
...
//check if all meals in order have been delivered
//if all meals are delivered:
//	update ordStat to 1
...
void checkOrderCompletion(MealOrder* MO)
{
	...
	// check if all meals in order have been delivered
	for(trav = MO->ML; trav != NULL && trav->mealStat != 0; trav = trav->link){}
	
	if(trav == NULL){ // meals in order fully delivered
		MO->ordStat = 1;
	}
}
...
// search for meal in order
// 
// if meal exists in order
// 	if meal not fully delivered:
// 		increase delivered qty
// 	else
// 		print message
//else
//	print message
...
void deliverMeals(MealOrder* MO, Meal DList[], int numMeals)
{
	...
	for(i = 0; i < numMeals; i++){ // traverse the DList one by one
		// search for meal in order
		for(trav = MO->ML; trav != NULL && strcmp(trav->order.mealCode, DList[i].mealCode) != 0; trav = trav->link){}
		
		if(trav != NULL){ // meal exists in order
			if(trav->delQty < trav->qty){ // meal not fully delivered
				trav->delQty++;
			} else {
			printf("\n%s has already been delivered.\n", trav->order.mealName);
			}
		} else { // meal does not exists in order
...
//	
//	else:
//		print system does not allow cancellation message
//else:
//	print error message
...
void removeMealFromOrder(MealOrder* MO, Meal cancelMeal)
{
	...
	// check if meal exists in order
	for(trav = &MO->ML; *trav != NULL && strcmp((*trav)->order.mealCode, cancelMeal.mealCode) != 0; trav = &(*trav)->link){}
	...
	if(*trav != NULL){ // meal exists in order
		if((*trav)->delQty < (*trav)->qty){ // meal not fully delivered
			...
			if((*trav)->delQty == (*trav)->qty){ // qty matches delQty after qty decremented
				(*trav)->mealStat = 1;
			}
			
			if((*trav)->qty <= 0){ // qty reaches 0
				temp = *trav;
				*trav = (*trav)->link;
		...
		} else { // meal fully delivered
			printf("\nAll of your %s have been delivered. I'm sorry we cannot cancel the order.\n", cancelMeal.mealName);
		}
	} else {
		printf("\nMeal does not exist in the order.\n");
	}
}
