...
{
	strcpy(BS->details.storeName, "National Bookstore");
	strcpy(BS->details.address, "Ayala Center Cebu");
	strcpy(BS->details.telephone, "+1-2345");
...
int findBook(bookStore BS, char bookID[])
{
	...
	// check if passed bookID or book is in bookstore
...
void insertBookBS(bookStore* BS, bookRecord newBk)
{
	...
	// check if book exists in book store, and insert in ascending order by book ID
	// if first string is greater, returns > 0
	// if second string is greater, returns < 0
	for(trav = &BS->bList; *trav != NULL && strcmp((*trav)->bkRec.bookID, newBk.bookID) < 0; trav = &(*trav)->nextBook){}

    if(*trav != NULL && strcmp((*trav)->bkRec.bookID, newBk.bookID) == 0){ // book already exists in book store
        return;
    }
	
	newNode = (BookList)malloc(sizeof(bookNode));
	newNode->bkRec = newBk;
	newNode->pages = 1;
...
void deleteBooksNoStock(bookStore* BS)
{
	...
	// check if any book record has zero stocks
	for(trav = &BS->bList; *trav != NULL && (*trav)->bkRec.stocks != 0; trav = &(*trav)->nextBook){}
	
	if(*trav != NULL){ // a book record has zero stocks
		temp = *trav;
		*trav = temp->nextBook;
...
custBkCollection initCustBkCollection()
{
	custBkCollection books = {{"Cris Militante", "+63-123", "clagmilitante@usc.edu.ph"}, NULL, 0};
...
{
	BookList bs_trav;
	PersonalBkList* cbc_trav;
	PersonalBkList newNode;

	// check if book that customer wants to purchase exists in bookstore
	for(bs_trav = BS->bList; bs_trav != NULL && strcmp(bs_trav->bkRec.bookID, bookID) != 0; bs_trav = bs_trav->nextBook){}
	
	if(bs_trav != NULL){ // book exists in bookstore
		// check if book exists in customer's collection
		for(cbc_trav = &CBC->bkList; *cbc_trav != NULL && strcmp((*cbc_trav)->bookName, bs_trav->bkRec.bookName) != 0; cbc_trav = &(*cbc_trav)->nextBk){}
		
		if(*cbc_trav != NULL){ // book already exists in customer's collection
			printf("You already have this book in your collection.\n");
		} else {
			newNode = (PersonalBkList)malloc(sizeof(personalBkNode));
			strcpy(newNode->bookName, bs_trav->bkRec.bookName);
			newNode->nextBk = NULL;
			
			*cbc_trav = newNode;
			
			bs_trav->bkRec.stocks--;
			CBC->count++;
			
			deleteBooksNoStock(BS);
		}
	} else {
		printf("Book does not exist in store.\n");
	}
}
