...
int main(void)
{
	...
	*/
	myCart = initCart();
	addToCart(&myCart, &groceryStore, prod1, 61);
...
Cart initCart(void)
{
	Cart myCart = {NULL, 0, "", {0.0, 0.0, 0.0}};
	return myCart;
}
...
{	
	...
	C->receipt.discount = C->receipt.totalPrice * promoList[0].discount;
...
void displayCart(Cart* C)
{
	...
	for(trav=C->sCart; trav!=NULL; trav=trav->nextItem){
		printf("\n%-10s%-15s%-10.2f%-10d%-15.2f", 
			...
			trav->item.unitPrice,
			trav->item.qty,
			(trav->item.unitPrice)*(trav->item.qty)
		);
	}
	genReceipt(C, promos, promos[0].promoCode);
	printf("\n\nNumber of items: %d\n\nPromo Code: %s\nVAT: %.2f\nDiscount: %.2f\nTotal Price: %.2f\n",
		C->numItems,
		C->promoCode,
		C->receipt.VAT,
...
void addToCart(Cart* C, ProdList* PL, char prodID[], int itemQty)
{
	...
	if(i<=PL->lastProd && (PL->prodList[i].qty-itemQty>=0)){
	
		insert(C, PL->prodList[i], itemQty);
		PL->prodList[i].qty -= itemQty;
	}else{
		printf("\nNot enough stocks to complete the transaction or not found\n");
	}
}

void insert(Cart* C, Product item, int itemQty){
	nodeItem* nn = (nodeItem*)malloc(sizeof(nodeItem));
	if(nn!=NULL){
		nn->item = item;
		nn->nextItem = C->sCart;
		C->sCart=nn;
...
void returnItem(Cart* C, ProdList* PL, char prodID[], int qty)
{
	...
	if(*trav!=NULL && ((*trav)->item.qty-qty>=0)){
		(*trav)->item.qty-=qty;
		if((*trav)->item.qty==0){
			SList temp = *trav;
			*trav = temp->nextItem;
